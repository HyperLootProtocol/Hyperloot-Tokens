let HyperLootTokenContract = artifacts.require("./HyperLootToken.sol");
let HLPMTTokenContract = artifacts.require("./HLPMTToken.sol");
...
contract('ManagementContract', async (accounts) => {
  ...
  let congress = accounts[8];
    ...
    beforeEach(async function() {
        ...
        tokenERC20 = await HyperLootTokenContract.deployed();
        minerToken = await HLPMTTokenContract.deployed();
    });
    ...
    it('should return correct erc721 balance', async function() {
        await managerContract.buyHLPMT({from: accounts[2]});
    ...
    it('should revert HLPMT buying', async function() {

        await tokenERC20.approve(managerContract.address, 100, {from: accounts[3]});
        await assertRevert(managerContract.buyHLPMT({from: accounts[3]}));
        await tokenERC20.approve(managerContract.address, 100000* 10 ** 18, {from: accounts[4]});
        await assertRevert(managerContract.buyHLPMT({from: accounts[4]}));
    });
